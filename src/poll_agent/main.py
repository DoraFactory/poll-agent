...
def main() -> int:
    ...
    if not settings.default_handles:
        logging.error("No handles configured: provide X_HANDLES in .env or environment variables.")
        return 1
    ...
    logging.info(
        "[service] started. x_handles=%s, interval=%ss, agent_model=%s, grok_model=%s",
