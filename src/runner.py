...
from tg_agent.agent import build_telegram_agent
...
def build_runner(settings: Settings) -> Runner:
    ...
    - telegram_agent: 负责将投票结果发送到 Telegram
    ...
    # 子 Agent：负责发送消息到 Telegram
    telegram_agent = build_telegram_agent(settings)

    # 协调器 Agent：负责 orchestrating poll generation
    orchestrator = build_poll_orchestrator(settings, sub_agents=[x_agent, telegram_agent])
